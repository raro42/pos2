...
| **Summary** | Total revenue (cents), total orders, and daily series (date, revenue, order count). |
...
| **Export Excel** | Downloads a workbook with sheets: Summary, By Product, By Category, By Table, By Waiter. |
...
- **GET `/reports/sales?from_date=YYYY-MM-DD&to_date=YYYY-MM-DD`**  
  Returns combined report: `summary`, `by_product`, `by_category`, `by_table`, `by_waiter`. Requires auth and `report:read`.
